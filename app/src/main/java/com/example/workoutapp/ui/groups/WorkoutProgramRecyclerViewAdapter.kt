...
class WorkoutProgramRecyclerViewAdapter : RecyclerView.Adapter<WorkoutProgramRecyclerViewAdapter.WorkoutGroupViewHolder> () {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WorkoutGroupViewHolder {
        return WorkoutGroupViewHolder(
            LayoutInflater.from(parent.context).inflate(R.layout.workout_program_card, parent, false)
        )
    }
    ...
    class WorkoutGroupViewHolder constructor(itemView: View): RecyclerView.ViewHolder(itemView) {
        val workoutGroupImage = itemView.findViewById<ImageView>(R.id.workout_program_image)
        val workoutGroupName = itemView.findViewById<TextView>(R.id.workout_program_name)
